...
require_once("User.php");
	...
	$res_comp = exec_query($sql);
	while($row = mysqli_fetch_assoc($res_comp)){
		$tot = sprintf('%01.2f', $row["tot_gen"]);
		$id_supp = $row["id_supp"];
	...
	$pdf->Ln(5);
	$pdf->SetFont('Arial','',9);
	$pdf->Cell(110,10,"Responsable: " . $person_name);
	...
	while($row = mysqli_fetch_assoc($res_pays)){	
		$pdf->Cell(35,7, "Payment conditions: ");
		$pdf->Cell(70,7,ucfirst($row["pay_cond"]));
		$pdf->Cell(35,7,"Expiration conditions:");
		$pdf->Cell(20,7, ucfirst($row["expiration_conditions"]));
		$pdf->Ln(5);
		$pdf->Cell(27,7,"Expiration days:");
		$pdf->Cell(78,7, $row["exp_days"]);
		...
		$pdf->Ln(5);
		$pdf->Cell(25,7,"Bank account:");
		$pdf->Cell(80,7, $row["bank_account"]);
		$pdf->Cell(20,7,"Branch:");
		$pdf->Cell(40,7, $row["branch"]);
		$pdf->Ln(5);
		$pdf->Cell(25,7,"Bank code:");
		$pdf->Cell(80,7, $row["bank_code"]);
		$pdf->Cell(20,7,"Bank key:");
		$pdf->Cell(40,7, $row["bank_key"]);
		$pdf->Ln(5);
		$pdf->Cell(25,7,"Swift:");
		$pdf->Cell(80,7, $row["swift"]);
		$pdf->Cell(20,7,"Clabe:");
		$pdf->Cell(40,7, $row["clabe"]);
	}
	$pdf->Output();
?>
